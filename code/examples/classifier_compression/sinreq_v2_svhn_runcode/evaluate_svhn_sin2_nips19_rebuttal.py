...
LINK: https://github.com/potterhsu/SVHNClassifier
...
headers = ['w11', 'w12', 'w13', 'w21', 'w22', 'w23', 'w31', 'w32', 'w33','w41', 'w42', 'w43']
with open('weight_traj.csv', 'w') as writeFile:
    writer = csv.writer(writeFile)
    writer.writerow(headers)
# ------------------------------------------------
def write_to_csv_w(step_data):
    with open('weight_traj.csv', 'a') as csvFile:
...
def eval_svhn_net(net_name, qbits_dict={}, layer_index=[], layer_name=[], trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=10, init=0):
	...
	if trainable:
		...
		with tf.Graph().as_default():
			...
			""" sin regularization """
			#cost_factor = 30.0 
			#sin2_func_0 = tf.Variable(0.0)
			#sin2_func_1 = tf.Variable(0.0)
			#sin2_func_2 = tf.Variable(0.0)
			#sin2_func_3 = tf.Variable(0.0)
			#sin2_func_4 = tf.Variable(0.0)
			#sin2_func_5 = tf.Variable(0.0)
			#sin2_func_6 = tf.Variable(0.0)
			#sin2_func_7 = tf.Variable(0.0)
			#sin2_func_8 = tf.Variable(0.0)
			#sin2_func_9 = tf.Variable(0.0)
			#sin2_func_10 = tf.Variable(0.0)
			...
			#print(netparams['weights']['hidden2'].shape)
			#print(netparams['weights']['hidden3'].shape)
			#print(netparams['weights']['hidden4'].shape)
			#print(netparams['weights']['hidden5'].shape)
			w11 = netparams['weights']['hidden2'][2][1][4][42]
			w12 = netparams['weights']['hidden2'][2][2][4][49]
			w13 = netparams['weights']['hidden2'][2][3][4][30]

			w21 = netparams['weights']['hidden3'][2][1][4][31]
			w22 = netparams['weights']['hidden3'][3][1][4][123]
			w23 = netparams['weights']['hidden3'][1][1][4][26]

			w31 = netparams['weights']['hidden4'][1][2][4][138]
			w32 = netparams['weights']['hidden4'][3][3][4][77]
			w33 = netparams['weights']['hidden4'][4][4][4][94]

			w41 = netparams['weights']['hidden5'][1][1][44][27]
			w42 = netparams['weights']['hidden5'][2][0][103][39]
			w43 = netparams['weights']['hidden5'][3][1][105][160]

			index = tf.argmax(netparams['weights']['hidden1'])
			...
			if  qbits < 8:
				...
				sin2_func_0 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
			...
			if  qbits < 8:
				...
				sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			else:
			...
			if  qbits < 8:
				...
				sin2_func_2 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			else:
			...
			if  qbits < 8:
				...
				sin2_func_3 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			...
			if  qbits < 8:
				...
				sin2_func_4 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			else:
			...
			if  qbits < 8:
				...
				sin2_func_5 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			else:
			...
			if  qbits < 8:
				...
				sin2_func_6 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			else:
			...
			if  qbits < 8:
				...
				sin2_func_7 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			else:
			...
			if  qbits < 8:
				...
				sin2_func_8 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			else:
			...
			if  qbits < 8:
				...
				sin2_func_9 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights'][layer_name]/(1/(2**(qbits)-1)))))
				#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
			else:
                                sin2_func_9 = tf.constant(0.0)

			""" ------------------------------------------------ """
			global_step = tf.Variable(0, name='global_step', dtype=float, trainable=False)
			training_options = {}
			...
			#loss_sin2_reg = cost_factor*(sin2_func_1 + sin2_func_2 +  sin2_func_3 + sin2_func_4)
                        ...
                        # dynamic setting 
			reg_cost = cost_factor*(1/1)*tf.exp(4*(global_step)/n_epoch)

			sinreq_loss = (sin2_func_0 + sin2_func_1 + sin2_func_2 +  sin2_func_3 + sin2_func_4 + sin2_func_5 + sin2_func_6 +  sin2_func_7 + sin2_func_8 + sin2_func_9)
			loss_sin2_reg = reg_cost*(sin2_func_0 + sin2_func_1 + sin2_func_2 +  sin2_func_3 + sin2_func_4 + sin2_func_5 + sin2_func_6 +  sin2_func_7 + sin2_func_8 + sin2_func_9)
			#loss_sin2_reg = (sin2_func_1)
			...
			with tf.Session() as sess:
				...
				for i in range(0, n_epoch): 
					...
					_, loss_val, loss_sin2_reg_val, global_step_val, cost_value, val, ind = sess.run([train_op, loss_op, sinreq_loss, global_step, reg_cost, value, index])
					aw11,aw12,aw13,aw21,aw22,aw23,aw31,aw32,aw33,aw41,aw42,aw43= sess.run([w11,w12,w13,w21,w22,w23,w31,w32,w33,w41,w42,w43])
					data = [aw11,aw12,aw13,aw21,aw22,aw23,aw31,aw32,aw33,aw41,aw42,aw43]
					write_to_csv_w(data)
...
qbits = {}
# keep Full Precision 
qbits['hidden1'] = 16  # layer 1

qbits['hidden2'] = 3   # layer 2
qbits['hidden3'] = 3   # layer 3
qbits['hidden4'] = 3   # layer 4
qbits['hidden5'] = 3   # layer 5

qbits['hidden6'] = 8   # layer 6
...
def write_to_csv_acc(step_data):
...
# csv file initialization:
headers = ['c_0.5', 'c_1.0', 'c_2.0', 'c_0']
with open('acc.csv', 'w') as writeFile:
    writer = csv.writer(writeFile)
    writer.writerow(headers)
