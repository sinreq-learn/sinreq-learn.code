...
def eval_imagenet(net_name, param_path, param_q_path, qbits, std, layer_index, layer_name, file_idx, shift_back, trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=1):
	...
	if trainable:
		...
		""" binary weight quantization, minmizing abs relative to the std of the weights distribution """
		# input layer 
		q_diff_0 = tf.subtract(std[0], netparams['weights']['conv1'])
		q_diff_0_cost = tf.nn.l2_loss(q_diff)	

		q_diff_1 = tf.subtract(std[1], netparams['weights']['conv2'])
		q_diff_1_cost = tf.nn.l2_loss(q_diff)	

		q_diff_2 = tf.subtract(std[2], netparams['weights']['conv3'])
		q_diff_2_cost = tf.nn.l2_loss(q_diff)	

		q_diff_3 = tf.subtract(std[3], netparams['weights']['conv4'])
		q_diff_3_cost = tf.nn.l2_loss(q_diff)	

		q_diff_4 = tf.subtract(std[4], netparams['weights']['conv5'])
		q_diff_4_cost = tf.nn.l2_loss(q_diff)	

		q_diff_5 = tf.subtract(std[5], netparams['weights']['fc6'])
		q_diff_5_cost = tf.nn.l2_loss(q_diff)	

		q_diff_6 = tf.subtract(std[6], netparams['weights']['fc7'])
		q_diff_6_cost = tf.nn.l2_loss(q_diff)	

		# last layer 
		q_diff_7 = tf.subtract(std[7], netparams['weights']['fc8'])
		q_diff_7_cost = tf.nn.l2_loss(q_diff)	


		loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits_, labels=label_node))+ cost_factor*(q_diff_1_cost+q_diff_2_cost+q_diff_3_cost+q_diff_4_cost+q_diff_5_cost+q_diff_6_cost)
	...
	with tf.Session() as sess:
		...
		if trainable:
			...
			image_producer = dataset.ImageNetProducer(val_path=path_val + '/val_20k_128bs.txt', data_path=path_val, data_spec=data_spec)
...
def run_inference(network, input_param_path, qbits):
	...
	std = []
	with tf.Graph().as_default():
		acc, netparams = eval_imagenet(network, param_path, param_q_path, qbits, std, layer_index, layer_name, file_idx, shift_back, trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=0., n_epoch=1)
	return acc
...
for i in range(len(qbits_combs)):
	...
	episode_num = 0
	layer_index = 1
	data = retrain(network_name, episode_num, layer_index, qbits, i)
	# writing to file (1)
