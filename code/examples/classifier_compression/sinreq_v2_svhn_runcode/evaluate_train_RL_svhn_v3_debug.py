...
def eval_lenet(net_name, param_path, qbits, layer_index, layer_name=[], trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=1):
	...
	with tf.Session() as sess:
		...
		for i in range(0, n_epoch):
			...
			#mnist.test.images[0] = np.load("numpyArray_mnist_single_28x28_image_flat.npy")
...
"""
print(qbits)
acc = eval_svhn_net(net_name='svhn_net', qbits_dict=qbits, trainable=True, n_epoch=100)
print(acc)
"""
"""
...
#qbits = [2] * 4
qbits = [32,32,32,32]
acc, acc_test = train_test_lenet(network_name, params, istrain, cost_factor, n_epoch, qbits, layer_index)
print('acc after taining')
print(acc)
print('acc after testing')
print(acc_test)
