...
#from .networks import alexnet, resnet18, lenet
from networks import helper
from networks import alexnet, resnet18, lenet
	...
	with tf.Session() as sess:
		...
		if trainable:
			...
			for i in range(0, n_epoch):
				...
				path_train = '/home/ahmed/projects/NN_quant/ILSVRC2012_img_train'
...
def train_network(net_name, istrain, cost_factor, n_epoch, qbits, layer_index):
	ckpt_path = CKPT_PATH + net_name + '/' + net_name + '.ckpt'
	err_mean =   [0.0, 0.0, 0.0, 0.0] #order: input, weights, biases, layers
	err_stddev = [0.0, 0.0, 0.0, 0.0]
	train_vars = [False, True, False, False]
	...
	if net_name == 'lenet':
		acc, netparams = eval_lenet(net_name=net_name, param_path=param_path, qbits=qbits, layer_index=layer_index, trainable=True, n_epoch=n_epoch)
	else:
		return eval_imagenet(net_name, ckpt_path, trainable=istrain, err_mean=err_mean, err_stddev=err_stddev, train_vars=train_vars, cost_factor=cost_factor, n_epoch=n_epoch)
...
def quantize_and_run_any(network, qbits):
	...
	#layers_sorted = load.get_layers('/backup/amir-tc/rl_quantization/rl_quantization.code/nn_quant_and_run_code/rlbitwidth.tfmodels/caffe2tf/tfmodels/alexnet/alexnet.py')
	layers_sorted = load.get_layers('./nn_quant_and_run_code/rlbitwidth.tfmodels/caffe2tf/tfmodels/'+ network +'/'+ network +'.py')
...
qbits = [2, 1, 2, 2]
train_network(net_name='lenet', istrain=True, cost_factor=0, n_epoch=10, qbits=qbits, layer_index=2)
save_path_params = 'lenet_retrained.pickle'
run_network('lenet', save_path_params, qbits, False, 0, 1)
