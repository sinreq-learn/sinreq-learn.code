...
def quantize_and_train(network_name, layer_index, layer_name, qbits, idx, init_params, file_idx):
	...
	with tf.Graph().as_default():
		acc, netparams = eval_imagenet(network_name, in_param_path, param_q_path, qbits, layer_index, layer_name, file_idx, shift_back, trainable=True, err_mean=None, err_stddev=None, train_vars=None, cost_factor=100., n_epoch=1)
	print(acc)

	""" path for saving the retrained model """
	path_save = HOME+'/nn_quant_and_run_code/results/quantized/' + network_name + '/' 
	retrained_path = path_save + 'retrained.pickle'
...
def run_inference(network, input_param_path, qbits):
	...
	#path_save_q = path_save + network +'_train_1_test_retrained_quantized.pickle'
	path_save_q = path_save + 'retrained_quantized.pickle'
	#layers_sorted = load.get_layers('/backup/amir-tc/rl_quantization/rl_quantization.code/nn_quant_and_run_code/rlbitwidth.tfmodels/caffe2tf/tfmodels/alexnet/alexnet.py')
	...
	#param_path = path_save_q
	param_path = retrained_model_quantized
	param_q_path = ''
	layer_index = 0
	layer_name = 0
	file_idx = 0
...
# start ========
qbits_combs=[]
#qbits_combs.append([32, 4, 4, 4, 4, 4, 4, 32])
#qbits_combs.append([32, 4, 4, 4, 4, 8, 8, 32])
...
#qbits_combs.append([8, 4,4,4,4,4,4, 8])
