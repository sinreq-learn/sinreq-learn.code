...
from networks import alexnet, resnet18, svhn_net, squeezenet
...
def eval_imagenet(net_name, param_path, param_q_path, qbits, layer_index, layer_name, file_idx, shift_back, trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=1):
	...
	if net_name == 'alexnet_noisy':
		...
		if trainable:
			logits_ = squeezenet.squeezenet_q(input_node, netparams, qbits)
		else:
			#logits_ = squeezenet.squeezenet_q(input_node, netparams, qbits)
			logits_ = squeezenet.squeezenet(input_node, netparams)
...
def eval_svhn_net(net_name, qbits_dict={}, layer_index=[], layer_name=[], trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=10):
	...
	if trainable:
		...
		with tf.Graph().as_default():
			...
			loss_op = length_cross_entropy + digit1_cross_entropy + digit2_cross_entropy + digit3_cross_entropy + digit4_cross_entropy + digit5_cross_entropy
...
def quantize_and_train(network_name, layer_index, layer_name, qbits, init_params, file_idx):
	...
	path_save_q = path_save + 'train_1_init_' + network_name + '_layers_quant_05Dec.pickle'
	layers_sorted = load.get_layers('./rlbitwidth.tfmodels/caffe2tf/tfmodels/' + network_name + '/' + network_name + '.py')
	...
	""" path for saving the retrained model """
	path_save = '../nn_quant_and_run_code/results/quantized/' + network_name + '/' + network_name
	path_save_params = path_save + '_train_1_layers_quant_retrained_05Dec_RL.pickle'
...
def run_inference(network, input_param_path, qbits):
	...
	#layers_sorted = load.get_layers('/backup/amir-tc/rl_quantization/rl_quantization.code/nn_quant_and_run_code/rlbitwidth.tfmodels/caffe2tf/tfmodels/alexnet/alexnet.py')
	layers_sorted = load.get_layers(CKPT_PATH+ network +'/'+ network +'.py')
	print(len(layers_sorted))
	#bits_q = [nbits] * len(layers_sorted)
	bits_q = qbits
...
episode_num = 0
layer_index = 1
qbits = [32, 4, 8, 8, 8, 8, 8, 32]
retrain(network_name, episode_num, layer_index, qbits)
"""

network_name = 'squeezenet'
ckpt_path = CKPT_PATH + network_name + '/' + network_name + '.ckpt'
qbits = [5] * 26 # squeezenet 
episode_num = 0
layer_index = 1
file_idx = 1
layer_name = []
acc = run_inference(network_name, ckpt_path, qbits)
path_save_params = quantize_and_train(network_name, layer_index, layer_name, qbits, ckpt_path, file_idx)
acc = run_inference(network_name, path_save_params, qbits)
print(acc)
