...
def quantize_and_train(qbits):
	...
	nbits = 16
	path_save = '../nn_quant_and_run_code/results/quantized/alexnet/'
	path_save_q = path_save + 'alexnet_layers_quant_'+ str(nbits) +'-bits_16Oct.pickle'
	#layers_sorted = load.get_layers('/backup/amir-tc/rl_quantization/rl_quantization.code/nn_quant_and_run_code/rlbitwidth.tfmodels/caffe2tf/tfmodels/alexnet/alexnet.py')
	...
	save_path_params = path_save + '_layers_quant_retrained_16Oct.pickle'


	acc, netparams = eval_imagenet('alexnet', param_path, shift_back, trainable=True, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=1)
	print(acc)
	# AHMED: debug
