...
def eval_imagenet(net_name, param_path, param_q_path, qbits, layer_index, layer_name, file_idx, shift_back, trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=1):
	...
	if trainable:
		...
		if qbits[1] < 8:
			#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['conv2']/(2**(-(qbits[1]))))))
			sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['conv2']/(2**(-(qbits[1]-1))))))
			#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1])))))) # WRONG
			#sin2_func_1 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv2']+2**-(qbits[1]))/(2**(-(qbits[1]-1))))))
		if qbits[2] < 8:
			#sin2_func_2 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['conv3']/(2**(-(qbits[2]))))))
			sin2_func_2 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['conv3']/(2**(-(qbits[2]-1))))))
			#sin2_func_2 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv3']+2**-(qbits[2]))/(2**(-(qbits[2]))))))
			#sin2_func_2 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv3']+2**-(qbits[2]))/(2**(-(qbits[2]-1))))))
		if qbits[3] < 8:
			#sin2_func_3 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['conv4']/(2**(-(qbits[3]))))))
			sin2_func_3 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['conv4']/(2**(-(qbits[3]-1))))))
			#sin2_func_3 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv4']+2**-(qbits[3]))/(2**(-(qbits[3]))))))
			#sin2_func_3 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv4']+2**-(qbits[3]))/(2**(-(qbits[3]-1))))))
		if qbits[4] < 8:
			#sin2_func_4 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['conv5']/(2**(-(qbits[4]))))))
			sin2_func_4 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['conv5']/(2**(-(qbits[4]-1))))))
			#sin2_func_4 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv5']+2**-(qbits[4]))/(2**(-(qbits[4]))))))
			#sin2_func_4 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['conv5']+2**-(qbits[4]))/(2**(-(qbits[4]-1))))))
		if qbits[5] < 8:
			#sin2_func_5 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['fc6']/(2**(-(qbits[5]))))))
			sin2_func_5 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['fc6']/(2**(-(qbits[5]-1))))))
			#sin2_func_5 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['fc6']+2**-(qbits[5]))/(2**(-(qbits[5]))))))
			#sin2_func_5 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['fc6']+2**-(qbits[5]))/(2**(-(qbits[5]-1))))))
		if qbits[6] < 8:
			#sin2_func_6 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['fc7']/(2**(-(qbits[6]))))))
			sin2_func_6 = tf.reduce_mean(tf.square(tf.sin(pi*netparams['weights']['fc7']/(2**(-(qbits[6]-1))))))
			#sin2_func_6 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['fc7']+2**-(qbits[6]))/(2**(-(qbits[6]))))))
			#sin2_func_6 = tf.reduce_mean(tf.square(tf.sin(pi*(netparams['weights']['fc7']+2**-(qbits[6]))/(2**(-(qbits[6]-1))))))

		cost_factor = 200.0 
		loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits_, labels=label_node))+ cost_factor*(sin2_func_1+sin2_func_2+sin2_func_3+sin2_func_4+sin2_func_5+sin2_func_6)

	probs  = helper.softmax(logits_)
	top_k_op = tf.nn.in_top_k(probs, label_node, 1)
	...
	with tf.Session() as sess:
		...
		if trainable:
			...
			for i in range(0, n_epoch):
				...
				#image_producer = dataset.ImageNetProducer(val_path=path_train + '/train_shuf_200k.txt', data_path=path_train, data_spec=data_spec)
			...
			#image_producer = dataset.ImageNetProducer(val_path=path_val + '/val_20k_20bs.txt', data_path=path_val, data_spec=data_spec)
...
def quantize_and_train(network_name, layer_index, layer_name, qbits, idx, init_params, file_idx):
	...
	with tf.Graph().as_default():
		acc, netparams = eval_imagenet(network_name, in_param_path, param_q_path, qbits, layer_index, layer_name, file_idx, shift_back, trainable=True, err_mean=None, err_stddev=None, train_vars=None, cost_factor=100., n_epoch=10)
	print(acc)

	""" path for saving the retrained model """
	path_save = HOME+'/nn_quant_and_run_code/results/quantized/' + network_name + '/' 
	retrained_path = path_save + 'retrained_FIRST.pickle'
...
def run_inference(network, input_param_path, qbits):
	...
	""" Quantization """
	# SECOND FILE: Quantized 
	path_save = HOME+'/nn_quant_and_run_code/results/quantized/'+ network +'/'
	path_save_q = path_save + network +'retrained_quantized_SECOND.pickle'
	#layers_sorted = load.get_layers('/backup/amir-tc/rl_quantization/rl_quantization.code/nn_quant_and_run_code/rlbitwidth.tfmodels/caffe2tf/tfmodels/alexnet/alexnet.py')
...
def retrain(network_name, episode_num, layer_index, qbits, idx):
	...
	acc_retrain = quantize_and_train(network_name, layer_index, layer_name, qbits, idx, init_params, file_idx)
	""" path for saving the retrained model """
	# FIRST FILE
	path_save = HOME+'/nn_quant_and_run_code/results/quantized/' + network_name + '/' 
	retrained_path = path_save + 'retrained_FIRST.pickle'
	print('[c] validation ACC after retraining')
	print('##########################################################')
	#qbits = [32] * 8
	acc_retrain_val = run_inference(network_name, retrained_path, qbits)
	acc_retrain_lst.append(acc_retrain)
	acc_retrain_val_lst.append(acc_retrain_val)
...
# start ========
qbits_combs=[]
#qbits_combs.append([32, 4, 4, 8, 8, 8, 8, 32])
#qbits_combs.append([32, 5, 8, 5, 8, 8, 8, 32])
#qbits_combs.append([32, 4, 8, 8, 4, 8, 8, 32])
#qbits_combs.append([32, 4, 8, 8, 8, 4, 8, 32])
#qbits_combs.append([32, 4, 8, 8, 8, 8, 4, 32])
#qbits_combs.append([32, 8, 4, 8, 4, 8, 4, 32])
#qbits_combs.append([2, 2, 2, 2, 4, 4, 4, 32])
#qbits_combs.append([8, 4, 4, 4, 4, 8, 8, 8])
#qbits_combs.append([4]*8)

# here 
qbits_combs.append([8,5,8,8,8,8,8,8])
#qbits_combs.append([8, 5,5,5,5,3,3, 8])
#qbits_combs.append([8, 5,5,5,3,3,3, 8])
#qbits_combs.append([8, 3,3,5,5,5,5, 8])
#qbits_combs.append([8, 3,3,3,3,5,5, 8])
#qbits_combs.append([8, 5,5,5,5,5,5, 8])
#qbits_combs.append([8, 3,3,3,3,3,3, 8])

def write_to_csv(step_data, file_label):
    with open('./retrain_slops_files_2/'+file_label+'.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(step_data)


# csv file (1) initialization:
file_label = 'retrain_reward_acc'
headers = ['acc_val','acc_retrain', 'acc_retrain_val']
with open('./retrain_slops_files_2/'+file_label+'.csv', 'w') as writeFile:
    writer = csv.writer(writeFile)
    writer.writerow(headers)

# for each quantization pattern 
for i in range(len(qbits_combs)):
	qbits = qbits_combs[i]

	file_label = 'retrain_profile'
	for j in range(len(qbits)):
		file_label=file_label+'_'+str(qbits[j])

	# csv file (2) initialization:
	...
	episode_num = 0
	layer_index = 1
	acc_data = retrain(network_name, episode_num, layer_index, qbits, i)
	# writing to file (1)
	data = []
	data = qbits
...
acc_val = run_inference(network_name, init_params, [32, 5, 8, 5, 8, 8, 8, 32])
print(acc_val)
"""
