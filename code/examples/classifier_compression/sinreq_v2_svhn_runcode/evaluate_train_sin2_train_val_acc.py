	...
	with tf.Session() as sess:
		...
		if trainable:
			...
			for i in range(0, n_epoch):
				...
				#image_producer = dataset.ImageNetProducer(val_path=path_train + '/train_shuf_test.txt', data_path=path_train, data_spec=data_spec)
...
def retrain(network_name, episode_num, layer_index, qbits, idx):
	...
	return acc_val, acc_retrain_lst, acc_retrain_val_lst
...
for i in range(len(qbits_combs)):
	...
	episode_num = 0
	layer_index = 1
	acc_val, retrain_lst, val_lst = retrain(network_name, episode_num, layer_index, qbits, i)
	# writing to file (1)
	write_to_csv(retrain_lst,'retrain_reward_acc')
	write_to_csv(val_lst,'retrain_reward_acc')
