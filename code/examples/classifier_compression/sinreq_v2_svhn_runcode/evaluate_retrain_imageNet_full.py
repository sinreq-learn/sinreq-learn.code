...
def quantize_and_train(network_name, layer_index, layer_name, qbits, init_params, file_idx):
	...
	with tf.Graph().as_default():
		acc, netparams = eval_imagenet(network_name, param_path, param_q_path, qbits, layer_index, layer_name, file_idx, shift_back, trainable=True, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=20)
	print(acc)
...
qbits_combs.append([8, 8, 4, 4, 8, 8, 8, 8])
...
for i in range(len(qbits_combs)):
	...
	write_to_csv(data,'retrain_acc_full')
