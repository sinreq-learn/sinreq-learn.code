...
from networks import alexnet, resnet18, vgg16net
...
def eval_imagenet(net_name, param_path, shift_back, qbits, trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=1):
	...
	if net_name == 'alexnet_noisy':
		...
		if trainable:
			logits_ = vgg16net.vgg16net(input_node, netparams)
			#logits_ = vgg16net.vgg16net_q(input_node, netparams, qbits)
		else:
			#logits_ = vgg16net.vgg16net(input_node, netparams)
			logits_ = vgg16net.vgg16net_q(input_node, netparams, qbits)
...
def quantize_and_train(qbits):
	...
	nbits = 16
	path_save = '../nn_quant_and_run_code/results/quantized/alexnet/'
	path_save_q = path_save + 'train_0_init_alexnet_layers_quant_16Oct.pickle'
	#layers_sorted = load.get_layers('/backup/amir-tc/rl_quantization/rl_quantization.code/nn_quant_and_run_code/rlbitwidth.tfmodels/caffe2tf/tfmodels/alexnet/alexnet.py')
	...
	# writing [netparams] after retraining 
	path_save = '../nn_quant_and_run_code/results/quantized/alexnet/alexnet'
	save_path_params = path_save + '_train_0_layers_quant_retrained_31Oct.pickle'

	acc, netparams = eval_imagenet('alexnet', param_path, shift_back, qbits=qbits, trainable=True, err_mean=None, err_stddev=None, train_vars=None, cost_factor=200., n_epoch=1)
	print(acc)
...
def run_inference(network, input_param_path, qbits):
	...
	if qbits:
		...
		nbits = 10
		path_save = '../nn_quant_and_run_code/results/quantized/'+ network +'/'
		path_save_q = path_save + network +'_train_0_test_retrained_quantized.pickle'
		#layers_sorted = load.get_layers('/backup/amir-tc/rl_quantization/rl_quantization.code/nn_quant_and_run_code/rlbitwidth.tfmodels/caffe2tf/tfmodels/alexnet/alexnet.py')
		layers_sorted = load.get_layers('./rlbitwidth.tfmodels/caffe2tf/tfmodels/'+ network +'/'+ network +'.py')
		#bits_q = [nbits] * len(layers_sorted)
		bits_q = qbits
	...
	with tf.Graph().as_default():
		acc, netparams = eval_imagenet(network, param_path, shift_back, qbits, trainable=False, err_mean=None, err_stddev=None, train_vars=None, cost_factor=0., n_epoch=1)
	return acc

def write_to_csv(step_data):
    with open('train_0_acc.csv', 'a') as csvFile:
...
headers = ['acc']
with open('train_0_acc.csv', 'w') as writeFile:
...
qbits = [32, 5, 8, 5, 8, 8, 8, 32]

network_name = 'vgg16net'
HOME = '/home/ahmed/projects/rl_quantization/rl_quantization.code'
path = HOME + '/nn_quant_and_run_code_train/rlbitwidth.tfmodels/caffe2tf/tfmodels/'+network_name+'/'+network_name+'.ckpt'
qbits = []
qbits = [8] * 16
acc = run_inference(network_name, path, qbits)
print(acc)
