...
def write_to_csv2(step_data):
    with open('sinreq_vgg11_losses_log.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(step_data)
# initializing acc_cache dict to use it as global var.
acc_cache = {}
headers = ['tot_loss','sin2_loss']
with open('sinreq_vgg11_losses_log.csv', 'w') as writeFile:
    writer = csv.writer(writeFile)
    writer.writerow(headers)
#--------------------------------------------------------------------
""" logging the accs """
def write_to_csv(step_data):
    with open('sinreq_vgg11_log.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(step_data)
# initializing acc_cache dict to use it as global var.
acc_cache = {}
headers = ['EPOCH','TOP1-ACC','TOP5-ACC','VAL-LOSS']
with open('sinreq_vgg11_accs_log.csv', 'w') as writeFile:
    writer = csv.writer(writeFile)
    writer.writerow(headers)
#--------------------------------------------------------------------
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    for train_step, (inputs, target) in enumerate(train_loader):
        ...
        if not args.earlyexit_lossweights:
            ...
            #Parameter 'features.module.3.weight' will be quantized to 4 bits
            #Parameter 'features.module.6.weight' will be quantized to 4 bits
            #Parameter 'features.module.8.weight' will be quantized to 4 bits
            #Parameter 'features.module.11.weight' will be quantized to 4 bits
            #Parameter 'features.module.13.weight' will be quantized to 4 bits
            #Parameter 'features.module.16.weight' will be quantized to 4 bits
            #Parameter 'features.module.18.weight' will be quantized to 4 bits
            ...
            #kernel = model.features.module[3].float_weight
            #print(kernel.data[0])

            kernel = model.features.module[4].float_weight
            sin2_func_2 = torch.mean(torch.pow(torch.sin(pi*(kernel+shift)/step),power)) # dorefa

            kernel = model.features.module[8].float_weight
            sin2_func_3 = torch.mean(torch.pow(torch.sin(pi*(kernel+shift)/step),power)) # dorefa

            kernel = model.features.module[11].float_weight
            sin2_func_4 = torch.mean(torch.pow(torch.sin(pi*(kernel+shift)/step),power)) # dorefa

            kernel = model.features.module[15].float_weight
            sin2_func_5 = torch.mean(torch.pow(torch.sin(pi*(kernel+shift)/step),power)) # dorefa

            kernel = model.features.module[18].float_weight
            sin2_func_6 = torch.mean(torch.pow(torch.sin(pi*(kernel+shift)/step),power)) # dorefa

            kernel = model.features.module[22].float_weight
            sin2_func_7 = torch.mean(torch.pow(torch.sin(pi*(kernel+shift)/step),power)) # dorefa

            kernel = model.features.module[25].float_weight
            sin2_func_1 = torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power) # dorefa
            #print(sin2_func_1.data[0])

            # ----------------------------------

            sin2_reg_loss = sin2_func_1 + sin2_func_2 + sin2_func_3 + sin2_func_4 + sin2_func_5 + sin2_func_6 + sin2_func_7 
