...
def write_to_csv2(step_data):
    with open('sinreq_cifar10_losses_bits.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(step_data)
# initializing acc_cache dict to use it as global var.
acc_cache = {}
headers = ['tot_loss','sin2_loss']
with open('sinreq_cifar10_losses_bits.csv', 'w') as writeFile:
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    #write_to_csv2([lambda_qp, lambda_fp, loss.data.cpu().numpy()[0], sin2_reg_loss.data.cpu().numpy(), model.module.B1.data.cpu().numpy()[0], model.module.B2.data.cpu().numpy()[0], model.module.B3.data.cpu().numpy()[0], model.module.B4.data.cpu().numpy()[0], model.module.B5.data.cpu().numpy()[0]])
