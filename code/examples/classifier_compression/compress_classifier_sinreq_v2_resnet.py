...
def write_to_csv2(step_data):
    with open('sin2Reg_resnet20_losses_log.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(step_data)
# initializing acc_cache dict to use it as global var.
acc_cache = {}
headers = ['tot_loss','sin2_loss']
with open('sin2Reg_resnet20_losses_log.csv', 'w') as writeFile:
    writer = csv.writer(writeFile)
    writer.writerow(headers)
#--------------------------------------------------------------------
""" logging the accs """
def write_to_csv(step_data):
    with open('sin2Reg_resnet20_log.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(step_data)
# initializing acc_cache dict to use it as global var.
acc_cache = {}
headers = ['EPOCH','TOP1-ACC','TOP5-ACC','VAL-LOSS']
with open('sin2Reg_resnet20_log.csv', 'w') as writeFile:
    writer = csv.writer(writeFile)
    writer.writerow(headers)
#--------------------------------------------------------------------
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    for train_step, (inputs, target) in enumerate(train_loader):
        ...
        if not args.earlyexit_lossweights:
            ...
            kernel = model.module.classifier[0].float_weight
            if (train_step == 0) and not (epoch % 2 == 0):
                w1 = kernel.data.cpu().numpy()
                np.save('weights_sin2Reg/svhn_L1_'+str(epoch)+'_'+str(train_step), w1)
                print('+++++++++++++++++++++++++++++++')
                print(train_step)
            # ---------------------
            ...
            # ahmed
            print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
            print((model.module.layer1[0].conv1.float_weight))
            print((model.module.layer2[0].downsample[0].float_weight))
            print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
            ...
            kernel = model.module.layer1[0].conv1.float_weight
            sin2_func_1 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            #kernel = model.module.layer1[0].conv1.weight
            #sin2_func_1 =((torch.sin(pi*(kernel+0)/(step)))**power)
            #print(sin2_func_1)

            kernel = model.module.layer1[0].conv2.float_weight
            sin2_func_2 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer1[1].conv1.float_weight
            sin2_func_3 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer1[1].conv2.float_weight
            sin2_func_4 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer1[2].conv1.float_weight
            sin2_func_5 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer1[2].conv2.float_weight
            sin2_func_6 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)


            kernel = model.module.layer2[0].conv1.float_weight
            sin2_func_7 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer2[0].conv2.float_weight
            sin2_func_8 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer2[1].conv1.float_weight
            sin2_func_9 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer2[1].conv2.float_weight
            sin2_func_10 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer2[2].conv1.float_weight
            sin2_func_11 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer2[2].conv2.float_weight
            sin2_func_12 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)


            kernel = model.module.layer3[0].conv1.float_weight
            sin2_func_13 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer3[0].conv2.float_weight
            sin2_func_14 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer3[1].conv1.float_weight
            sin2_func_15 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer3[1].conv2.float_weight
            sin2_func_16 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer3[2].conv1.float_weight
            sin2_func_17 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer3[2].conv2.float_weight
            sin2_func_18 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)



            kernel = model.module.layer2[0].downsample[0].float_weight
            sin2_func_19 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            kernel = model.module.layer3[0].downsample[0].float_weight
            sin2_func_20 =torch.mean((torch.sin(pi*(kernel+shift)/(step)))**power)

            sin2_reg_loss = sin2_func_1 + sin2_func_2 + sin2_func_3 + sin2_func_4 + sin2_func_5 + sin2_func_6 + sin2_func_7 + sin2_func_8 \
                           + sin2_func_9 + sin2_func_10 + sin2_func_11 + sin2_func_12 + sin2_func_13 + sin2_func_14 + sin2_func_15 + sin2_func_16 \
                           + sin2_func_17 + sin2_func_18 + sin2_func_19 + sin2_func_20 
